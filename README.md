...
├──────────────────────────────────────────────────────────────────────┤
│  代理: 留空            [开始] [停止]  剩余: 35 | 已读: 15 | 用时: 2:30│
├──────────────┬───────────────────────────────────────────────────────┤
│  板块选择    │  运行日志                                             │
...
```bash
uv run python linux_do_gui.py
```

macOS 用户也可以运行仓库内的启动脚本：

```bash
./LinuxDoHelper.command
```
...
- 程序会优先使用 `/Applications/Google Chrome.app/Contents/MacOS/Google Chrome` 启动浏览器，并兼容常见的 Chromium、Microsoft Edge、Brave 路径。
- 代理默认留空；只有本机代理端口已经启动时才需要填写，例如 `127.0.0.1:7897`。
- 如遇到打包版窗口空白，建议从源码通过 `uv` 隔离环境运行。
...
### v8.5 (2026-05-01)
- 🍎 **修复 macOS 源码运行**：使用主线程获取屏幕高度，避免点击开始后后台线程创建 Tk 窗口导致界面异常
- 🌐 **修复 Chrome 启动路径**：macOS 下自动定位 Google Chrome/Chromium/Edge/Brave 可执行文件，避免 `Permission denied: 'chrome'`
- 🔌 **默认不启用代理**：代理输入框默认留空，避免本机代理端口未启动时浏览器白屏
- 📦 **补充 uv 隔离运行说明**：推荐使用 `uv` 创建独立环境运行源码版
